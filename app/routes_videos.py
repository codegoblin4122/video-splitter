import os, uuid, math, subprocess, tempfile, shutil
from fastapi import APIRouter, UploadFile, File, Depends, HTTPException, Query
from fastapi.responses import JSONResponse
from .config import validate_cloud_env
from .aws import (
...
def list_videos(user=Depends(require_user)):
    items = ddb_list_videos(user["email"])
    # reshape DDB wire format -> JSON
    out = []
    for it in items:
        out.append({
...
def split_sync(video_id: str, mode: str = Query("heavy"), parts: int = Query(10, ge=2, le=100),
               user=Depends(require_user)):
    return _do_split(video_id, mode, parts, user["email"], async_=False)

@router.post("/videos/{video_id}/split_async")
def split_async(video_id: str, mode: str = Query("heavy"), parts: int = Query(10, ge=2, le=100),
                user=Depends(require_user)):
    # record job and return immediately; you can run a background task/thread/celery here
    job_id = str(uuid.uuid4())
    ddb_put_job(user["email"], job_id, video_id, mode, parts, "queued")
    # fire-and-forget background
    import threading
    threading.Thread(target=_do_split, args=(video_id, mode, parts, user["email"], True, job_id), daemon=True).start()
...
def job_status(job_id: str, user=Depends(require_user)):
    ...
    # coerce numeric
    if "parts" in out: out["parts"] = int(out["parts"])
    if "parts_done" in out: out["parts_done"] = int(out["parts_done"])
    if "updated_at" in out: out["updated_at"] = int(out["updated_at"])
    return out

def _do_split(video_id: str, mode: str, parts: int, user_email: str, async_: bool, job_id: str|None=None):
    if job_id:
    ...
    if not v:
        if job_id: ddb_update_job_status(user_email, job_id, "error")
    ...
    try:
        ...
        groups = [{"mode": "heavy" if mode!="fast" else "fast",
                   "parts": len(uploaded),
...
def list_segments(video_id: str, user=Depends(require_user)):
    ...
    out = []
    for mode, prefix in [("fast", f"{user['email']}/{video_id}/segments_fast/"),
                         ("heavy", f"{user['email']}/{video_id}/segments_heavy/")]:
        # naive probe: try first 2 parts; if found, assume up to 100 parts and stop at gaps
        urls = []
        for i in range(100):
            key = f"{prefix}part_{i:02d}.mp4"
            try:
                # HEAD via presign & short curl isn’t trivial here; just presign and let client 403 if missing
